...
namespace WPDeskInvoicesVendor\WPDesk\Helper\Integration;

use WPDeskInvoicesVendor\Monolog\Logger;
use WPDeskInvoicesVendor\WPDesk\Helper\Page\SettingsPage;
use WPDeskInvoicesVendor\WPDesk\Logger\WPDeskLoggerFactory;
 ...
 */
class LogsIntegration implements \WPDeskInvoicesVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    /** @var Logger */
    private $logger;
    public function __construct(\WPDeskInvoicesVendor\WPDesk\Helper\Page\SettingsPage $settings_page)
    {
    ...
    {
        $logger_factory = new \WPDeskInvoicesVendor\WPDesk\Logger\WPDeskLoggerFactory();
        ...
        if (\property_exists($logger_factory, 'shouldLoggerBeActivated')) {
            ...
            $this->logger = new \WPDeskInvoicesVendor\Monolog\Logger('fallback');
        }
    }
    private function maybe_show_logging_notice()
    {
        ...
        if ($debug_log_enabled) {
            if (\apply_filters('wpdesk_helper_show_log_notices_library', \true)) {
                new \WPDeskInvoicesVendor\WPDesk\Notice\Notice(\sprintf(
                    // Translators: link.
                    ...
                    '</a>'
                ), \WPDeskInvoicesVendor\WPDesk\Notice\Notice::NOTICE_TYPE_INFO);
            }
        }
    }
