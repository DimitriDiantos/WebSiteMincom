...
if (!\class_exists('WPDeskInvoicesVendor\\WPDesk_Tracker_Sender_Logged')) {
    class WPDesk_Tracker_Sender_Logged implements \WPDesk_Tracker_Sender
    {
        ...
        {
            if (\class_exists('WPDeskInvoicesVendor\\WPDesk_Logger_Factory')) {
                \WPDeskInvoicesVendor\WPDesk_Logger_Factory::log_message("Sender payload: " . \json_encode($payload), self::LOGGER_SOURCE, \WPDeskInvoicesVendor\WPDesk_Logger::DEBUG);
                try {
                    $response = $this->sender->send_payload($payload);
                    \WPDeskInvoicesVendor\WPDesk_Logger_Factory::log_message('Sender response: ' . \json_encode($response), self::LOGGER_SOURCE, \WPDeskInvoicesVendor\WPDesk_Logger::DEBUG);
                    return $response;
                } catch (\WPDeskInvoicesVendor\WPDesk_Tracker_Sender_Exception_WpError $exception) {
                    \WPDeskInvoicesVendor\WPDesk_Logger_Factory::log_exception($exception);
                    throw $exception;
                }
            } else {
