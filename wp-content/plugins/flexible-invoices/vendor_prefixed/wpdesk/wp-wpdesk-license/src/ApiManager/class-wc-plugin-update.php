 ...
 *
 */
if (!\class_exists('WPDeskInvoicesVendor\\WPDesk_Update_API_Check')) {
    class WPDesk_Update_API_Check
    {
        ...
        public function plugin_information($args)
        {
            ...
            if (\is_wp_error($request) || \wp_remote_retrieve_response_code($request) != 200) {
                if (\class_exists('WPDeskInvoicesVendor\\WPDesk_Logger_Factory')) {
                    if (\is_wp_error($request)) {
                        \WPDeskInvoicesVendor\WPDesk_Logger_Factory::log_wp_error($request, \debug_backtrace());
                    } else {
                        \WPDeskInvoicesVendor\WPDesk_Logger_Factory::log_message_backtrace('Response is invalid. Response: ' . \json_encode($request), \WPDeskInvoicesVendor\WPDesk_Logger::ERROR, \debug_backtrace());
                    }
                }
                return \false;
            }
            ...
            if (\is_object($response)) {
                ...
                if (\class_exists('WPDeskInvoicesVendor\\WPDesk_Logger_Factory')) {
                    \WPDeskInvoicesVendor\WPDesk_Logger_Factory::log_message_backtrace('Response is not an object', \WPDeskInvoicesVendor\WPDesk_Logger::DEBUG, \debug_backtrace());
                } else {
