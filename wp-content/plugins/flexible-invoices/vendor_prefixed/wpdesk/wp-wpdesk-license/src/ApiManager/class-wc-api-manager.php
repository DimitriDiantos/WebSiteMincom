...
use WPDeskInvoicesVendor\WPDesk\Notice\AjaxHandler;
use WPDeskInvoicesVendor\WPDesk\Notice\PermanentDismissibleNotice;
if (!\defined('ABSPATH')) {
    exit;
}
if (!\class_exists('WPDeskInvoicesVendor\\WPDesk_API_Manager_With_Update_Flag')) {
    class WPDesk_API_Manager_With_Update_Flag
    {
        ...
        public function create_instance_id()
        {
            ...
            $password_management = new \WPDeskInvoicesVendor\WPDesk_API_Password_Management();
        ...
        public function key()
        {
            if ($this->key_insance == null) {
                $this->key_insance = new \WPDeskInvoicesVendor\WPDesk_API_KEY($this);
            }
        ...
        public function update_check($upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra = '')
        {
            if ($this->update_check_insance == null) {
                $this->update_check_insance = new \WPDeskInvoicesVendor\WPDesk_Update_API_Check($this, $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra);
            }
        ...
        public function inactive_notice()
        {
            ...
            if (\apply_filters('wpdesk_show_plugin_activation_notice', \true)) {
                (new \WPDeskInvoicesVendor\WPDesk\Notice\AjaxHandler())->hooks();
                new \WPDeskInvoicesVendor\WPDesk\Notice\PermanentDismissibleNotice(\sprintf(\__('The %s%s%s API Key has not been activated, so you won\'t be supported and your plugin won\'t be updated! %sClick here%s to activate the API key and the plugin.', $this->text_domain), '<strong>', $this->software_product_id, '</strong>', '<a href="' . \esc_url(\admin_url('admin.php?page=wpdesk-licenses')) . '">', '</a>'), "notice-{$this->software_product_id}", \WPDeskInvoicesVendor\WPDesk\Notice\PermanentDismissibleNotice::NOTICE_TYPE_WARNING);
            }
        }
        /**
        ...
        public function check_external_blocking()
        {
            ...
            if (\defined('WPDeskInvoicesVendor\\WP_HTTP_BLOCK_EXTERNAL') && \WPDeskInvoicesVendor\WP_HTTP_BLOCK_EXTERNAL === \true) {
                // check if our API endpoint is in the allowed hosts
                $host = \parse_url($this->upgrade_url, \PHP_URL_HOST);
                if (!\defined('WPDeskInvoicesVendor\\WP_ACCESSIBLE_HOSTS') || \stristr(\WPDeskInvoicesVendor\WP_ACCESSIBLE_HOSTS, $host) === \false) {
                    ?>
                    <div class="error">
                        <p><?php 
