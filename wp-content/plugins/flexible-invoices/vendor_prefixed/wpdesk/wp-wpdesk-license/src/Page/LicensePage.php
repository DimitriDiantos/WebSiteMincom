 ...
 */
class LicensePage implements \WPDeskInvoicesVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    public function __construct(\WPDeskInvoicesVendor\WPDesk\License\InstalledPlugins $plugin_database)
    {
    ...
    public function handle_render_wpdesk_licenses_page()
    {
        ...
        if (!\class_exists('WPDeskInvoicesVendor\\WPDesk_Helper_List_Table')) {
    ...
    private function execute_plugin_action($plugin, $action)
    {
        ...
        if (isset($plugin_info)) {
            (new \WPDeskInvoicesVendor\WPDesk\License\Page\LicensePageActions())->create_action($action)->execute($plugin_info);
        }
    }
    /**
