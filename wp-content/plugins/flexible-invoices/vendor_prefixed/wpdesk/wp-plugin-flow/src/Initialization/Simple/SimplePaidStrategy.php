...
namespace WPDeskInvoicesVendor\WPDesk\Plugin\Flow\Initialization\Simple;

use WPDeskInvoicesVendor\WPDesk\Helper\HelperRemover;
use WPDeskInvoicesVendor\WPDesk\Helper\PrefixedHelperAsLibrary;
use WPDeskInvoicesVendor\WPDesk\License\PluginRegistrator;
use WPDeskInvoicesVendor\WPDesk\Plugin\Flow\Initialization\ActivationTrait;
use WPDeskInvoicesVendor\WPDesk\Plugin\Flow\Initialization\BuilderTrait;
use WPDeskInvoicesVendor\WPDesk\Plugin\Flow\Initialization\PluginDisablerByFileTrait;
use WPDeskInvoicesVendor\WPDesk\Plugin\Flow\Initialization\InitializationStrategy;
use WPDeskInvoicesVendor\WPDesk\PluginBuilder\Plugin\ActivationAware;
 ...
 */
class SimplePaidStrategy implements \WPDeskInvoicesVendor\WPDesk\Plugin\Flow\Initialization\InitializationStrategy
{
    ...
    /** @var SlimPlugin */
    private $plugin;
    public function __construct(\WPDeskInvoicesVendor\WPDesk_Plugin_Info $plugin_info)
    {
     ...
     * @return SlimPlugin
     */
    public function run_before_init(\WPDeskInvoicesVendor\WPDesk_Plugin_Info $plugin_info)
    {
     ...
     * @return SlimPlugin
     */
    public function run_init(\WPDeskInvoicesVendor\WPDesk_Plugin_Info $plugin_info)
    {
        if (!$this->plugin) {
            $this->plugin = $this->build_plugin($plugin_info);
        }
        ...
        \add_action('plugins_loaded', function () use($registrator) {
            $this->init_helper();
            $is_plugin_subscription_active = $registrator instanceof \WPDeskInvoicesVendor\WPDesk\License\PluginRegistrator && $registrator->is_active();
            if ($this->plugin instanceof \WPDeskInvoicesVendor\WPDesk\PluginBuilder\Plugin\ActivationAware && $is_plugin_subscription_active) {
    ...
    private function register_plugin()
    {
        if (\apply_filters('wpdesk_can_register_plugin', \true, $this->plugin_info)) {
            $registrator = new \WPDeskInvoicesVendor\WPDesk\License\PluginRegistrator($this->plugin_info);
    ...
    private function prevent_older_helpers()
    {
        ...
        if (\apply_filters('wpdesk_can_remove_old_helper_hooks', \true, $this->plugin_info)) {
            (new \WPDeskInvoicesVendor\WPDesk\Helper\HelperRemover())->hooks();
        }
    }
    /**
    ...
    {
        (new \WPDeskInvoicesVendor\WPDesk\Plugin\Flow\Initialization\PluginDisablerByFileTrait('wpdesk-helper/wpdesk-helper.php'))->disable();
    }
}
