...
namespace WPDeskInvoicesVendor\WPDesk\Logger;

use Exception;
use InvalidArgumentException;
use LogicException;
use WPDeskInvoicesVendor\Monolog\Handler\NullHandler;
use WPDeskInvoicesVendor\Monolog\Logger;
use WPDeskInvoicesVendor\Monolog\Registry;
use WPDeskInvoicesVendor\Monolog\Handler\StreamHandler;
use Psr\Log\LogLevel;
use WPDeskInvoicesVendor\WPDesk\Logger\WC\WooCommerceCapture;
use WPDeskInvoicesVendor\WPDesk\Logger\WP\WPCapture;
/**
 * Manages and facilitates creation of logger
 *
 * @package WPDesk\Logger
 */
class WPDeskLoggerFactory extends \WPDeskInvoicesVendor\WPDesk\Logger\BasicLoggerFactory
{
    ...
    {
        if (\WPDeskInvoicesVendor\Monolog\Registry::hasLogger($name)) {
            \WPDeskInvoicesVendor\Monolog\Registry::removeLogger($name);
        }
    }
    /**
    ...
    public function disableLog($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        if (\WPDeskInvoicesVendor\Monolog\Registry::hasLogger($name)) {
            /** @var Logger $logger */
            $logger = \WPDeskInvoicesVendor\Monolog\Registry::getInstance($name);
    ...
    public function createWPDeskLogger($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (!self::$shouldLoggerBeActivated) {
            return new \WPDeskInvoicesVendor\Monolog\Logger($name);
        }
        if (\WPDeskInvoicesVendor\Monolog\Registry::hasLogger($name)) {
            return \WPDeskInvoicesVendor\Monolog\Registry::getInstance($name);
        }
    ...
    private function captureWPLog()
    {
        static $wpCapture;
        if (!$wpCapture) {
            $wpCapture = new \WPDeskInvoicesVendor\WPDesk\Logger\WP\WPCapture(\basename($this->getFileName()));
     ...
     * @param Logger $logger
     */
    private function captureWooCommerce(\WPDeskInvoicesVendor\Monolog\Logger $logger)
    {
        if (!\defined('WPDeskInvoicesVendor\\WC_LOG_THRESHOLD')) {
            \define('WPDeskInvoicesVendor\\WC_LOG_THRESHOLD', self::LEVEL_WC);
        }
        $wcIntegration = new \WPDeskInvoicesVendor\WPDesk\Logger\WC\WooCommerceCapture($logger);
     ...
     */
    private function pushFileHandle($filename, \WPDeskInvoicesVendor\Monolog\Logger $logger)
    {
        $logger->pushHandler(new \WPDeskInvoicesVendor\Monolog\Handler\StreamHandler($filename, self::LEVEL_WPDESK_FILE));
    }
    /**
    ...
    public function getFileName($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        return \trailingslashit(\untrailingslashit($upload_dir['basedir'])) . \WPDeskInvoicesVendor\WPDesk\Logger\WP\WPCapture::LOG_DIR . \DIRECTORY_SEPARATOR . $name . '_debug.log';
    }
    /**
     ...
     * @return void
     */
    private function removeAllHandlers(\WPDeskInvoicesVendor\Monolog\Logger $logger)
    {
        try {
            while (\true) {
                $logger->popHandler();
            ...
            $logger->pushHandler(new \WPDeskInvoicesVendor\Monolog\Handler\NullHandler());
        }
    }
    /**
    ...
    {
        return \WPDeskInvoicesVendor\Monolog\Registry::hasLogger($name);
    }
}
