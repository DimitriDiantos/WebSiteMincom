...
use Exception;
use WPDeskInvoicesVendor\Monolog\Formatter\LineFormatter;
use WPDeskInvoicesVendor\Monolog\Logger;
use WPDeskInvoicesVendor\Monolog\Utils;
use WPDeskInvoicesVendor\PhpConsole\Connector;
use WPDeskInvoicesVendor\PhpConsole\Handler;
use WPDeskInvoicesVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \WPDeskInvoicesVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \WPDeskInvoicesVendor\PhpConsole\Connector $connector = null, $level = \WPDeskInvoicesVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('WPDeskInvoicesVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\WPDeskInvoicesVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \WPDeskInvoicesVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \WPDeskInvoicesVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\WPDeskInvoicesVendor\PhpConsole\Helper::isRegistered()) {
            \WPDeskInvoicesVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \WPDeskInvoicesVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \WPDeskInvoicesVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \WPDeskInvoicesVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \WPDeskInvoicesVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
