 ...
 */
namespace WPDeskInvoicesVendor\Monolog;

use WPDeskInvoicesVendor\Monolog\Handler\HandlerInterface;
use WPDeskInvoicesVendor\Monolog\Handler\StreamHandler;
use Psr\Log\LoggerInterface;
use WPDeskInvoicesVendor\Psr\Log\InvalidArgumentException;
use Exception;
/**
 * Monolog log channel
 *
 ...
 */
class Logger implements \Psr\Log\LoggerInterface, \WPDeskInvoicesVendor\Monolog\ResettableInterface
{
    /**
     ...
     * @return $this
     */
    public function pushHandler(\WPDeskInvoicesVendor\Monolog\Handler\HandlerInterface $handler)
    {
    ...
    {
        if (!$this->handlers) {
            $this->pushHandler(new \WPDeskInvoicesVendor\Monolog\Handler\StreamHandler('php://stderr', static::DEBUG));
        }
    ...
    {
        foreach ($this->handlers as $handler) {
            if ($handler instanceof \WPDeskInvoicesVendor\Monolog\ResettableInterface) {
                $handler->reset();
            }
        }
        foreach ($this->processors as $processor) {
            if ($processor instanceof \WPDeskInvoicesVendor\Monolog\ResettableInterface) {
                $processor->reset();
            }
        }
    }
    ...
    public static function getLevelName($level)
    {
        if (!isset(static::$levels[$level])) {
            throw new \WPDeskInvoicesVendor\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels)));
        }
