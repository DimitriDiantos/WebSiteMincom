<?php
    ...
    function getProductsNamesFromDataBase() {
		$productsFromDb = get_posts( array('post_type' => 'product', 'posts_per_page' => -1) );

		$productsNames = [];

		foreach($productsFromDb as $productFromDb) {
			$productsNames[] = $productFromDb->post_title;
		}
	...
	function getProductPrice($productName) {
		...
		foreach($productsFromDb as $productFromDb) {
			$productsNames[$productFromDb->post_title] = $productFromDb->ID;
		}
		$product = wc_get_product($productsNames[$productName]);

		return $product->get_price();
	}

	function getShippingMethods() {
		$shippingZones = class_exists('WC_Shipping_Zones')? WC_Shipping_Zones::get_zones() : array();
		...
		foreach ($shippingZones as $shippingZone) {
			...
			foreach($shippingZone["shipping_methods"] as $shippingMethod) {
				...
				if (get_class($shippingMethod) === "WPDesk_Flexible_Shipping") {
					foreach($shippingMethod->get_all_rates() as $rate) {

						if (!in_array($rate["method_title"], $shippingMethods)) {
							$shippingMethods[] = $rate["method_title"];

							if ($rate["method_free_shipping_label"] !== "") {
								$shippingMethods[] = $rate["method_title"] . " (" . $rate["method_free_shipping_label"] . ")";
							}
						}
					}
				}
	...
	function getShippingMethodPrice($methodName) {
		$shippingZones = class_exists('WC_Shipping_Zones')? WC_Shipping_Zones::get_zones() : array();
			...
			foreach($shippingZone["shipping_methods"] as $shippingMethod) {
				...
				if (get_class($shippingMethod) === "WPDesk_Flexible_Shipping") {
					foreach($shippingMethod->get_all_rates() as $rate) {
						...
						if (!in_array($rate["method_title"], $shippingMethods)) {
							foreach($rate["method_rules"] as $methodRule) {
								$shippingMethods[$rate["method_title"]] = $methodRule["cost_per_order"];
							}

							if ($rate["method_free_shipping_label"] !== "") {
								$title = $rate["method_title"] . " (" . $rate["method_free_shipping_label"] . ")";
								$shippingMethods[$title] = "0";
							}
						}
					}
				}
		...
		return isset($shippingMethods[$methodName])? $shippingMethods[$methodName]: "0";
	}

	function productExistsInDatabase($product) {
		return in_array($product, getProductsNamesFromDataBase()) || in_array($product, getShippingMethods());
	}

	function isSelected ($productFromDb, $currentProduct) {
		if ($productFromDb === $currentProduct) {
			return 'selected';
		}
	}
			...
			<?php if (!empty($products)): ?>
				<?php foreach ($products as $index => $product): ?>
					<tr class="product_row">
						<td>
<!--							<input type="text" name="product[name][]" value="--><?php //echo esc_attr( @$product['name'] ); ?><!--" />-->
							<?php if(productExistsInDatabase(@$product['name'])) { ?>
								<select name="product[name][]" class="refresh_product wide-input">
									<optgroup label="Produkty">
										<?php foreach(getProductsNamesFromDataBase() as $productName) { ?>
											<option
												value="<?php echo esc_attr($productName); ?>"
												<?php echo isSelected($productName, @$product['name']); ?>
												data-price="<?php echo getProductPrice($productName); ?>"
											>
												<?php echo esc_attr($productName); ?>
											</option>
										<?php } ?>
									</optgroup>
									<optgroup label="Rodzaje dostawy">
										<?php foreach(getShippingMethods() as $method) { ?>
											<option
												value="<?php echo esc_attr($method); ?>"
												<?php echo isSelected($method, @$product['name']); ?>
												data-price="<?php echo getShippingMethodPrice($method); ?>"
											>
