 ...
 */
class Flexible_Invoices_Plugin extends \WPDeskInvoicesVendor\WPDesk\PluginBuilder\Plugin\AbstractPlugin implements \WPDeskInvoicesVendor\WPDesk\PluginBuilder\Plugin\Activateable {
	 ...
	 *
	 * @param WPDeskInvoicesVendor\WPDesk_Plugin_Info $plugin_info Plugin data.
	 */
	...
	public function admin_enqueue_scripts() {
		...
		if ( in_array( $current_screen->id, array( 'inspire_invoice', 'edit-inspire_invoice', 'inspire_invoice_page_invoices_settings' ), true ) ) {
		...
		if ( in_array( $current_screen->id, array( 'inspire_invoice', 'edit-inspire_invoice', 'inspire_invoice_page_invoices_settings', 'edit-shop_order', 'shop_order' ), true ) ) {
			...
			wp_enqueue_script( 'inspire-invoice-admin', $this->get_plugin_url() . 'assets/js/admin.js', array( 'jquery', 'jquery-ui-datepicker' ), $this->script_version, true );
		...
		if ( $this->select2_visibility() ) {
			...
			if ( defined( 'WC_VERSION' ) && version_compare( WC_VERSION, '3.1', '>' ) ) {
				...
				wp_enqueue_script( 'fi-select2-script', '//cdnjs.cloudflare.com/ajax/libs/select2/4.0.3/js/select2.min.js', array( 'jquery', 'jquery-ui-sortable' ), $this->script_version, true );
			}
		}
		...
		if ( 'inspire_invoice_page_invoices_settings' === $current_screen->id ) {
			...
			wp_enqueue_script( 'invoice_wc_settings_js', $this->get_plugin_assets_url() . '/js/woocommerce/settings.min.js', array( 'jquery', 'jquery-ui-color', 'jquery-ui-sortable' ), $this->script_version, true );
			...
			wp_enqueue_script( 'invoice_wc_admin', $this->get_plugin_assets_url() . '/js/woocommerce/woocommerce_admin.min.js', array( 'jquery', 'jquery-ui-sortable' ), $this->script_version, true );
