...
class Invoice {
	...
	public function loadTemplate($name, $path = '', $args = array()) {
		...
		$path = trim( $path, '/') ;
		if ( file_exists( $templateName = implode( '/', apply_filters( 'flexible_invoices_template_location', array( get_stylesheet_directory(), $this->getTemplatePath(), $path, $name . '.php') ) ) ) ) {
		} else {
