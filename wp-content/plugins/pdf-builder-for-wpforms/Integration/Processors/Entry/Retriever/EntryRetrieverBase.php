...
abstract class EntryRetrieverBase
{
    ...
    {
        global $wpdb;
        $recordData= $wpdb->get_results($wpdb->prepare('select fields, entry,seq_num,date,raw from '.$this->Loader->RECORDS_TABLE.' record join '.$this->Loader->FormConfigTable.' form on form.id=record.form_id where record.id=%d' ,$entryId));
     ...
     */
    public function InitializeByEntryItems($entryItems,$raw=null)
    {
    ...
    public function GetHtmlByFieldId($fieldId)
    {
